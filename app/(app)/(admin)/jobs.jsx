import {
  StyleSheet,
  View,
  FlatList,
  RefreshControl,
  TouchableOpacity,
} from "react-native";
import React, {
  useEffect,
  useState,
  useRef,
  useMemo,
  useCallback,
} from "react";
import { Stack, useRouter } from "expo-router";
import { useSelector, useDispatch } from "react-redux";
import {
  Text,
  Card,
  Chip,
  Button,
  IconButton,
  Divider,
  RadioButton,
  Badge,
} from "react-native-paper";
import { Ionicons, MaterialCommunityIcons } from "@expo/vector-icons";
import {
  fetchAllJobs,
  clearSuccess,
  clearError,
} from "../../../redux/slices/jobSlice";
import BottomSheet, {
  BottomSheetView,
  BottomSheetScrollView,
  BottomSheetBackdrop,
} from "@gorhom/bottom-sheet";
import { useSafeAreaInsets } from "react-native-safe-area-context";

// Skeleton Card Component
const SkeletonCard = () => (
  <Card style={styles.jobCard}>
    <Card.Content style={styles.cardContent}>
      <View style={styles.jobHeader}>
        <View style={[styles.skeleton, styles.skeletonTitle]} />
        <View style={[styles.skeleton, styles.skeletonChip]} />
      </View>
      <View style={[styles.skeleton, styles.skeletonHrInfo]} />
      <View style={[styles.skeleton, styles.skeletonDescription]} />
      <View style={[styles.skeleton, styles.skeletonDescription2]} />
      <View style={styles.jobMeta}>
        <View style={[styles.skeleton, styles.skeletonMeta]} />
        <View style={[styles.skeleton, styles.skeletonMeta]} />
        <View style={[styles.skeleton, styles.skeletonMeta]} />
      </View>
      <View style={styles.skillsContainer}>
        <View style={[styles.skeleton, styles.skeletonSkill]} />
        <View style={[styles.skeleton, styles.skeletonSkill]} />
        <View style={[styles.skeleton, styles.skeletonSkill]} />
      </View>
    </Card.Content>
  </Card>
);

const Jobs = () => {
  const router = useRouter();
  const dispatch = useDispatch();
  const { bottom } = useSafeAreaInsets();

  const { jobs, loading, pagination, success, error } = useSelector(
    (state) => state.jobs
  );
  const { token } = useSelector((state) => state.auth);

  // State
  const [searchQuery, setSearchQuery] = useState("");
  const [appliedStatus, setAppliedStatus] = useState("all");
  const [appliedLocation, setAppliedLocation] = useState("");
  const [tempStatus, setTempStatus] = useState("all");
  const [tempLocation, setTempLocation] = useState("");
  const [currentPage, setCurrentPage] = useState(1);
  const [refreshing, setRefreshing] = useState(false);
  const [selectedJob, setSelectedJob] = useState(null);
  const [isInitialLoad, setIsInitialLoad] = useState(true);

  // Bottom Sheets
  const filterBottomSheetRef = useRef(null);
  const detailsBottomSheetRef = useRef(null);
  const filterSnapPoints = useMemo(() => ["50%", "75%"], []);
  const detailsSnapPoints = useMemo(() => ["70%", "90%"], []);

  // Backdrop component
  const renderBackdrop = useCallback(
    (props) => (
      <BottomSheetBackdrop
        {...props}
        opacity={0.5}
        enableTouchThrough={false}
        appearsOnIndex={0}
        disappearsOnIndex={-1}
        style={[
          { backgroundColor: "rgba(0, 0, 0, 1)" },
          StyleSheet.absoluteFillObject,
        ]}
      />
    ),
    []
  );

  // Filters
  const statusFilters = [
    { label: "All Jobs", value: "all" },
    { label: "Published", value: "published" },
    { label: "Draft", value: "draft" },
    { label: "Closed", value: "closed" },
  ];

  const locationFilters = [
    { label: "All Locations", value: "" },
    { label: "Remote", value: "remote" },
    { label: "Hyderabad", value: "hyderabad" },
    { label: "Bangalore", value: "bangalore" },
    { label: "Mumbai", value: "mumbai" },
    { label: "Delhi", value: "delhi" },
    { label: "Pune", value: "pune" },
  ];

  // Load jobs
  useEffect(() => {
    loadJobs();
  }, [appliedStatus, appliedLocation, currentPage]);

  // Handle success/error
  useEffect(() => {
    if (success) {
      setTimeout(() => dispatch(clearSuccess()), 3000);
    }
    if (error) {
      setTimeout(() => dispatch(clearError()), 3000);
    }
  }, [success, error]);

  const loadJobs = () => {
    const params = {
      page: currentPage,
      limit: 10,
      token,
    };

    if (appliedStatus !== "all") {
      params.status = appliedStatus;
    }

    if (appliedLocation) {
      params.location = appliedLocation;
    }

    dispatch(fetchAllJobs(params));
    setIsInitialLoad(false);
  };

  const onRefresh = async () => {
    setRefreshing(true);
    setCurrentPage(1);
    await loadJobs();
    setRefreshing(false);
  };

  // Open filter sheet
  const handleOpenFilters = () => {
    setTempStatus(appliedStatus);
    setTempLocation(appliedLocation);
    filterBottomSheetRef.current?.expand();
  };

  // Apply filters
  const handleApplyFilters = () => {
    setAppliedStatus(tempStatus);
    setAppliedLocation(tempLocation);
    setCurrentPage(1);
    filterBottomSheetRef.current?.close();
  };

  // Clear filters
  const handleClearFilters = () => {
    setTempStatus("all");
    setTempLocation("");
  };

  // View Job Details
  const handleViewJobDetails = useCallback((job) => {
    setSelectedJob(job);
    detailsBottomSheetRef.current?.expand();
  }, []);

  const handleViewApplications = () => {
    detailsBottomSheetRef.current?.close();
    router.push(`/(admin)/applications?jobId=${selectedJob._id}`);
  };

  // Get status color
  const getStatusColor = (status) => {
    switch (status) {
      case "published":
        return "#10b981";
      case "draft":
        return "#f59e0b";
      case "closed":
        return "#ef4444";
      default:
        return "#6b7280";
    }
  };

  // Check if filters are active
  const hasActiveFilters = appliedStatus !== "all" || appliedLocation !== "";

  // Render Job Card
  const renderJobCard = useCallback(
    ({ item }) => (
      <Card style={styles.jobCard}>
        <Card.Content style={styles.cardContent}>
          {/* Header with Title, Status, and View Icon */}
          <View style={styles.jobHeader}>
            <View style={styles.jobHeaderLeft}>
              <Text
                variant="titleMedium"
                style={styles.jobTitle}
                numberOfLines={2}
              >
                {item.title}
              </Text>
            </View>
            <View style={styles.jobHeaderRight}>
              <Chip
                mode="flat"
                compact
                textStyle={styles.statusChipText}
                style={[
                  styles.statusChip,
                  { backgroundColor: getStatusColor(item.status) },
                ]}
              >
                {item.status.toUpperCase()}
              </Chip>
              <IconButton
                icon="eye"
                size={20}
                iconColor="#6200ee"
                style={styles.viewIcon}
                onPress={() => handleViewJobDetails(item)}
              />
            </View>
          </View>

          {/* Posted By (HR Manager) - Enhanced */}
          <View style={styles.postedByContainer}>
            <View style={styles.hrBadge}>
              <MaterialCommunityIcons
                name="account-tie"
                size={16}
                color="#6200ee"
              />
            </View>
            <View style={styles.hrDetailsContainer}>
              <Text variant="bodyMedium" style={styles.hrName}>
                {item.createdBy?.name || "Unknown HR"}
              </Text>
              <Text variant="bodySmall" style={styles.hrRole}>
                {item.createdBy?.role?.toUpperCase() || "HR"} •{" "}
                {item.createdBy?.email || "No email"}
              </Text>
            </View>
          </View>

          {/* Description */}
          <Text
            variant="bodyMedium"
            numberOfLines={2}
            style={styles.description}
          >
            {item.description}
          </Text>

          {/* Meta Information */}
          <View style={styles.jobMeta}>
            <View style={styles.metaItem}>
              <Ionicons name="location-outline" size={14} color="#5f6368" />
              <Text variant="bodySmall" style={styles.metaText}>
                {item.location || "Remote"}
              </Text>
            </View>
            <View style={styles.metaItem}>
              <Ionicons name="briefcase-outline" size={14} color="#5f6368" />
              <Text variant="bodySmall" style={styles.metaText}>
                {item.experience?.min}-{item.experience?.max} yrs
              </Text>
            </View>
            <View style={styles.metaItem}>
              <Ionicons name="time-outline" size={14} color="#5f6368" />
              <Text variant="bodySmall" style={styles.metaText}>
                {new Date(item.createdAt).toLocaleDateString("en-US", {
                  month: "short",
                  day: "numeric",
                })}
              </Text>
            </View>
          </View>

          {/* Applications Count Badge */}
          {item.applicationsCount > 0 && (
            <View style={styles.applicationsCountContainer}>
              <Badge size={20} style={styles.applicationsBadge}>
                {item.applicationsCount || 0}
              </Badge>
              <Text variant="bodySmall" style={styles.applicationsText}>
                Applications
              </Text>
            </View>
          )}

          {/* Skills */}
          {item.requiredSkills && item.requiredSkills.length > 0 && (
            <View style={styles.skillsContainer}>
              {item.requiredSkills.slice(0, 4).map((skill, index) => (
                <Chip
                  key={index}
                  mode="flat"
                  compact
                  style={styles.skillChip}
                  textStyle={styles.skillChipText}
                >
                  {skill}
                </Chip>
              ))}
              {item.requiredSkills.length > 4 && (
                <Chip
                  mode="flat"
                  compact
                  style={styles.skillChip}
                  textStyle={styles.skillChipText}
                >
                  +{item.requiredSkills.length - 4}
                </Chip>
              )}
            </View>
          )}
        </Card.Content>
      </Card>
    ),
    [handleViewJobDetails]
  );

  // Render skeleton loader
  const renderSkeletons = () => (
    <>
      {[1, 2, 3, 4, 5].map((item) => (
        <SkeletonCard key={item} />
      ))}
    </>
  );

  return (
    <View style={[styles.container]}>
      <Stack.Screen
        options={{
          headerShown: true,
          headerTitle: "All Jobs",
          headerStyle: {
            backgroundColor: "#fff",
          },
          headerShadowVisible: true,
          headerRight: () => (
            <View style={styles.headerRightContainer}>
              <IconButton
                icon="filter-variant"
                size={24}
                iconColor={hasActiveFilters ? "#6200ee" : "#666"}
                onPress={handleOpenFilters}
                style={hasActiveFilters && styles.filterIconActive}
              />
            </View>
          ),
          headerSearchBarOptions: {
            placeholder: "Search jobs by title...",
            hideWhenScrolling: false,
            autoCapitalize: "none",
            barTintColor: "#fff",
            hintTextColor: "#888",
            onChangeText: setSearchQuery,
          },
        }}
      />

      {/* Active Filters Indicator */}
      {hasActiveFilters && (
        <View style={styles.activeFiltersContainer}>
          <View style={styles.activeFiltersContent}>
            {appliedStatus !== "all" && (
              <Chip
                mode="flat"
                onClose={() => {
                  setAppliedStatus("all");
                  setCurrentPage(1);
                }}
                style={styles.activeFilterChip}
              >
                {statusFilters.find((f) => f.value === appliedStatus)?.label}
              </Chip>
            )}
            {appliedLocation && (
              <Chip
                mode="flat"
                onClose={() => {
                  setAppliedLocation("");
                  setCurrentPage(1);
                }}
                style={styles.activeFilterChip}
              >
                {
                  locationFilters.find((f) => f.value === appliedLocation)
                    ?.label
                }
              </Chip>
            )}
          </View>
          <Button
            mode="text"
            compact
            onPress={() => {
              setAppliedStatus("all");
              setAppliedLocation("");
              setCurrentPage(1);
            }}
            textColor="#f44336"
          >
            Clear All
          </Button>
        </View>
      )}

      {/* Jobs List with Loading State */}
      {loading && isInitialLoad ? (
        <View style={styles.listContent}>{renderSkeletons()}</View>
      ) : jobs.length === 0 ? (
        <View style={styles.emptyContainer}>
          <MaterialCommunityIcons
            name="briefcase-off-outline"
            size={80}
            color="#ccc"
          />
          <Text variant="titleMedium" style={styles.emptyTitle}>
            No Jobs Found
          </Text>
          <Text variant="bodyMedium" style={styles.emptySubtitle}>
            No job postings available at the moment
          </Text>
        </View>
      ) : (
        <FlatList
          data={jobs}
          renderItem={renderJobCard}
          keyExtractor={(item) => item._id}
          contentContainerStyle={styles.listContent}
          refreshControl={
            <RefreshControl
              refreshing={refreshing}
              onRefresh={onRefresh}
              colors={["#6200ee"]}
            />
          }
          onEndReached={() => {
            if (pagination.page < pagination.pages && !loading) {
              setCurrentPage(currentPage + 1);
            }
          }}
          onEndReachedThreshold={0.5}
          ListFooterComponent={
            loading && !isInitialLoad ? (
              <View style={styles.loadingFooter}>
                <Text>Loading more...</Text>
              </View>
            ) : null
          }
        />
      )}

      {/* Filter Bottom Sheet */}
      <BottomSheet
        ref={filterBottomSheetRef}
        index={-1}
        snapPoints={filterSnapPoints}
        enablePanDownToClose
        backgroundStyle={styles.bottomSheetBackground}
        backdropComponent={renderBackdrop}
      >
        <BottomSheetScrollView style={styles.filterSheetContent}>
          <View style={styles.filterSheetHeader}>
            <Text variant="titleLarge" style={styles.filterSheetTitle}>
              Filter Jobs
            </Text>
            <View style={styles.filterHeaderActions}>
              <Button
                mode="contained"
                onPress={handleApplyFilters}
                compact
                style={styles.applyButton}
                labelStyle={styles.applyButtonLabel}
              >
                Apply
              </Button>
              <IconButton
                icon="close"
                size={24}
                onPress={() => filterBottomSheetRef.current?.close()}
                style={styles.closeButton}
              />
            </View>
          </View>

          {/* Status Filter */}
          <View style={styles.filterSection}>
            <Text variant="titleMedium" style={styles.filterSectionTitle}>
              Job Status
            </Text>
            <RadioButton.Group
              onValueChange={(value) => setTempStatus(value)}
              value={tempStatus}
            >
              {statusFilters.map((filter) => (
                <TouchableOpacity
                  key={filter.value}
                  style={styles.radioItem}
                  onPress={() => setTempStatus(filter.value)}
                >
                  <RadioButton.Android value={filter.value} color="#6200ee" />
                  <Text variant="bodyLarge">{filter.label}</Text>
                </TouchableOpacity>
              ))}
            </RadioButton.Group>
          </View>

          <Divider style={styles.sectionDivider} />

          {/* Location Filter */}
          <View style={styles.filterSection}>
            <Text variant="titleMedium" style={styles.filterSectionTitle}>
              Location
            </Text>
            <RadioButton.Group
              onValueChange={(value) => setTempLocation(value)}
              value={tempLocation}
            >
              {locationFilters.map((filter) => (
                <TouchableOpacity
                  key={filter.value}
                  style={styles.radioItem}
                  onPress={() => setTempLocation(filter.value)}
                >
                  <RadioButton.Android value={filter.value} color="#6200ee" />
                  <Text variant="bodyLarge">{filter.label}</Text>
                </TouchableOpacity>
              ))}
            </RadioButton.Group>
          </View>

          {/* Clear All Button at Bottom */}
          <View style={styles.filterBottomActions}>
            <Button
              mode="text"
              onPress={handleClearFilters}
              textColor="#f44336"
              icon="close-circle-outline"
            >
              Clear All Filters
            </Button>
          </View>
        </BottomSheetScrollView>
      </BottomSheet>

      {/* Job Details Bottom Sheet (View Only) */}
      <BottomSheet
        ref={detailsBottomSheetRef}
        index={-1}
        snapPoints={detailsSnapPoints}
        enablePanDownToClose
        backgroundStyle={styles.bottomSheetBackground}
        backdropComponent={renderBackdrop}
      >
        <BottomSheetScrollView style={styles.detailsSheetContent}>
          {/* Job Title and Status */}
          <View style={styles.detailsHeader}>
            <Text
              variant="headlineSmall"
              style={styles.detailsTitle}
              numberOfLines={3}
            >
              {selectedJob?.title}
            </Text>
            <Chip
              mode="flat"
              compact
              textStyle={styles.statusChipText}
              style={[
                styles.statusChip,
                {
                  backgroundColor: getStatusColor(selectedJob?.status),
                },
              ]}
            >
              {selectedJob?.status?.toUpperCase()}
            </Chip>
          </View>

          {/* HR Information Card - Enhanced */}
          {selectedJob?.createdBy && (
            <Card style={styles.hrInfoCard}>
              <Card.Content style={styles.hrInfoCardContent}>
                <View style={styles.hrInfoHeader}>
                  <MaterialCommunityIcons
                    name="account-tie"
                    size={24}
                    color="#6200ee"
                  />
                  <Text variant="titleMedium" style={styles.hrInfoTitle}>
                    Posted By
                  </Text>
                </View>

                <View style={styles.hrInfoDetails}>
                  <View style={styles.hrInfoRow}>
                    <Ionicons name="person" size={18} color="#6b7280" />
                    <View style={styles.hrInfoTextContainer}>
                      <Text variant="bodySmall" style={styles.hrInfoLabel}>
                        Name
                      </Text>
                      <Text variant="bodyMedium" style={styles.hrInfoValue}>
                        {selectedJob.createdBy?.name || "Unknown"}
                      </Text>
                    </View>
                  </View>

                  <View style={styles.hrInfoRow}>
                    <Ionicons name="mail" size={18} color="#6b7280" />
                    <View style={styles.hrInfoTextContainer}>
                      <Text variant="bodySmall" style={styles.hrInfoLabel}>
                        Email
                      </Text>
                      <Text variant="bodyMedium" style={styles.hrInfoValue}>
                        {selectedJob.createdBy?.email || "N/A"}
                      </Text>
                    </View>
                  </View>

                  <View style={styles.hrInfoRow}>
                    <Ionicons
                      name="shield-checkmark"
                      size={18}
                      color="#6b7280"
                    />
                    <View style={styles.hrInfoTextContainer}>
                      <Text variant="bodySmall" style={styles.hrInfoLabel}>
                        Role
                      </Text>
                      <Text variant="bodyMedium" style={styles.hrInfoValue}>
                        {selectedJob.createdBy?.role?.toUpperCase() || "HR"}
                      </Text>
                    </View>
                  </View>

                  {selectedJob.createdBy?.phone && (
                    <View style={styles.hrInfoRow}>
                      <Ionicons name="call" size={18} color="#6b7280" />
                      <View style={styles.hrInfoTextContainer}>
                        <Text variant="bodySmall" style={styles.hrInfoLabel}>
                          Phone
                        </Text>
                        <Text variant="bodyMedium" style={styles.hrInfoValue}>
                          {selectedJob.createdBy.phone}
                        </Text>
                      </View>
                    </View>
                  )}
                </View>
              </Card.Content>
            </Card>
          )}

          <Divider style={styles.detailsDivider} />

          {/* Description */}
          <View style={styles.detailsSection}>
            <Text variant="titleMedium" style={styles.detailsSectionTitle}>
              Description
            </Text>
            <Text variant="bodyMedium" style={styles.detailsText}>
              {selectedJob?.description}
            </Text>
          </View>

          {/* Job Info */}
          <View style={styles.detailsSection}>
            <Text variant="titleMedium" style={styles.detailsSectionTitle}>
              Job Details
            </Text>
            <View style={styles.detailsInfoGrid}>
              <View style={styles.detailsInfoItem}>
                <Ionicons name="location" size={18} color="#6200ee" />
                <View>
                  <Text variant="bodySmall" style={styles.detailsInfoLabel}>
                    Location
                  </Text>
                  <Text variant="bodyMedium" style={styles.detailsInfoValue}>
                    {selectedJob?.location || "Remote"}
                  </Text>
                </View>
              </View>

              <View style={styles.detailsInfoItem}>
                <Ionicons name="briefcase" size={18} color="#6200ee" />
                <View>
                  <Text variant="bodySmall" style={styles.detailsInfoLabel}>
                    Experience
                  </Text>
                  <Text variant="bodyMedium" style={styles.detailsInfoValue}>
                    {selectedJob?.experience?.min}-
                    {selectedJob?.experience?.max} years
                  </Text>
                </View>
              </View>

              {selectedJob?.salary && (
                <View style={styles.detailsInfoItem}>
                  <Ionicons name="cash" size={18} color="#6200ee" />
                  <View>
                    <Text variant="bodySmall" style={styles.detailsInfoLabel}>
                      Salary
                    </Text>
                    <Text variant="bodyMedium" style={styles.detailsInfoValue}>
                      ₹{selectedJob.salary.min}-{selectedJob.salary.max}
                    </Text>
                  </View>
                </View>
              )}

              <View style={styles.detailsInfoItem}>
                <Ionicons name="calendar" size={18} color="#6200ee" />
                <View>
                  <Text variant="bodySmall" style={styles.detailsInfoLabel}>
                    Posted On
                  </Text>
                  <Text variant="bodyMedium" style={styles.detailsInfoValue}>
                    {new Date(selectedJob?.createdAt).toLocaleDateString(
                      "en-US",
                      {
                        month: "short",
                        day: "numeric",
                        year: "numeric",
                      }
                    )}
                  </Text>
                </View>
              </View>
            </View>
          </View>

          {/* Required Skills */}
          {selectedJob?.requiredSkills &&
            selectedJob.requiredSkills.length > 0 && (
              <View style={styles.detailsSection}>
                <Text variant="titleMedium" style={styles.detailsSectionTitle}>
                  Required Skills
                </Text>
                <View style={styles.detailsSkillsContainer}>
                  {selectedJob.requiredSkills.map((skill, index) => (
                    <Chip
                      key={index}
                      mode="flat"
                      style={styles.detailsSkillChip}
                      textStyle={styles.detailsSkillChipText}
                    >
                      {skill}
                    </Chip>
                  ))}
                </View>
              </View>
            )}

          {/* View Applications Button */}
          <View style={styles.detailsActions}>
            <Button
              mode="contained"
              icon="account-group"
              onPress={handleViewApplications}
              style={styles.viewApplicationsButton}
            >
              View Applications
            </Button>
            <Text variant="bodySmall" style={styles.readOnlyNote}>
              * Admin has view-only access to job postings
            </Text>
          </View>
        </BottomSheetScrollView>
      </BottomSheet>
    </View>
  );
};

export default Jobs;

const styles = StyleSheet.create({
  container: {
    flex: 1,
    backgroundColor: "#f8f9fa",
  },
  headerRightContainer: {
    flexDirection: "row",
    alignItems: "center",
    marginRight: 8,
  },
  filterIconActive: {
    backgroundColor: "#f0e6ff",
  },
  activeFiltersContainer: {
    flexDirection: "row",
    alignItems: "center",
    justifyContent: "space-between",
    paddingHorizontal: 16,
    paddingVertical: 12,
    backgroundColor: "#fff",
    borderBottomWidth: 1,
    borderBottomColor: "#e0e0e0",
  },
  activeFiltersContent: {
    flexDirection: "row",
    flexWrap: "wrap",
    gap: 8,
    flex: 1,
  },
  activeFilterChip: {
    backgroundColor: "#f0e6ff",
  },
  listContent: {
    padding: 12,
  },
  jobCard: {
    marginBottom: 12,
    backgroundColor: "#fff",
    borderRadius: 12,
    elevation: 2,
    shadowColor: "#000",
    shadowOffset: { width: 0, height: 1 },
    shadowOpacity: 0.1,
    shadowRadius: 3,
  },
  cardContent: {
    padding: 14,
  },
  jobHeader: {
    flexDirection: "row",
    justifyContent: "space-between",
    alignItems: "flex-start",
    marginBottom: 8,
    gap: 10,
  },
  jobHeaderLeft: {
    flex: 1,
  },
  jobHeaderRight: {
    flexDirection: "row",
    alignItems: "center",
    gap: 4,
  },
  jobTitle: {
    fontWeight: "700",
    color: "#111827",
    fontSize: 16,
    lineHeight: 22,
  },
  statusChip: {
    height: 30,
    borderRadius: 6,
  },
  statusChipText: {
    color: "#ffffff",
    fontSize: 10,
    fontWeight: "700",
    letterSpacing: 0.5,
  },
  viewIcon: {
    margin: 0,
    marginTop: -4,
  },
  // Enhanced Posted By Container
  postedByContainer: {
    flexDirection: "row",
    alignItems: "center",
    gap: 10,
    marginBottom: 12,
    padding: 10,
    backgroundColor: "#f0f4ff",
    borderRadius: 8,
    borderLeftWidth: 3,
    borderLeftColor: "#6200ee",
  },
  hrBadge: {
    width: 32,
    height: 32,
    borderRadius: 16,
    backgroundColor: "#ffffff",
    justifyContent: "center",
    alignItems: "center",
    elevation: 1,
    shadowColor: "#000",
    shadowOffset: { width: 0, height: 1 },
    shadowOpacity: 0.1,
    shadowRadius: 2,
  },
  hrDetailsContainer: {
    flex: 1,
  },
  hrName: {
    color: "#111827",
    fontWeight: "700",
    fontSize: 14,
    marginBottom: 2,
  },
  hrRole: {
    color: "#6b7280",
    fontSize: 11,
    fontWeight: "500",
  },
  description: {
    color: "#6b7280",
    marginBottom: 10,
    lineHeight: 20,
    fontSize: 14,
  },
  jobMeta: {
    flexDirection: "row",
    flexWrap: "wrap",
    gap: 12,
    marginBottom: 10,
  },
  metaItem: {
    flexDirection: "row",
    alignItems: "center",
    gap: 4,
  },
  metaText: {
    color: "#6b7280",
    fontSize: 13,
    fontWeight: "500",
  },
  applicationsCountContainer: {
    flexDirection: "row",
    alignItems: "center",
    gap: 6,
    marginBottom: 10,
  },
  applicationsBadge: {
    backgroundColor: "#6200ee",
  },
  applicationsText: {
    color: "#6b7280",
    fontSize: 12,
  },
  skillsContainer: {
    flexDirection: "row",
    flexWrap: "wrap",
    gap: 6,
    marginTop: 2,
  },
  skillChip: {
    height: 30,
    backgroundColor: "#ede9fe",
    borderRadius: 6,
  },
  skillChipText: {
    fontSize: 11,
    color: "#7c3aed",
    fontWeight: "600",
  },
  centerContainer: {
    flex: 1,
    justifyContent: "center",
    alignItems: "center",
  },
  emptyContainer: {
    flex: 1,
    justifyContent: "center",
    alignItems: "center",
    paddingHorizontal: 32,
  },
  emptyTitle: {
    marginTop: 16,
    fontWeight: "600",
    color: "#1a1a1a",
  },
  emptySubtitle: {
    marginTop: 8,
    color: "#999",
    textAlign: "center",
  },
  loadingFooter: {
    paddingVertical: 20,
    alignItems: "center",
  },
  bottomSheetBackground: {
    backgroundColor: "#fff",
    borderTopLeftRadius: 0,
    borderTopRightRadius: 0,
  },
  filterSheetContent: {
    flex: 1,
    paddingHorizontal: 20,
  },
  filterSheetHeader: {
    flexDirection: "row",
    justifyContent: "space-between",
    alignItems: "center",
    marginBottom: 20,
    paddingTop: 8,
  },
  filterSheetTitle: {
    fontWeight: "600",
    color: "#1a1a1a",
    flex: 1,
  },
  filterHeaderActions: {
    flexDirection: "row",
    alignItems: "center",
    gap: 4,
  },
  applyButton: {
    backgroundColor: "#6200ee",
    borderRadius: 8,
  },
  applyButtonLabel: {
    fontSize: 14,
    fontWeight: "600",
  },
  closeButton: {
    margin: 0,
  },
  filterSection: {
    marginBottom: 20,
  },
  filterSectionTitle: {
    fontWeight: "600",
    marginBottom: 12,
    color: "#1a1a1a",
  },
  radioItem: {
    flexDirection: "row",
    alignItems: "center",
    paddingVertical: 8,
  },
  sectionDivider: {
    marginVertical: 16,
  },
  filterBottomActions: {
    alignItems: "center",
    paddingVertical: 20,
    paddingBottom: 40,
  },
  detailsSheetContent: {
    flex: 1,
    paddingHorizontal: 20,
    paddingTop: 8,
  },
  detailsHeader: {
    marginBottom: 16,
  },
  detailsTitle: {
    fontWeight: "700",
    color: "#111827",
    marginBottom: 12,
  },
  // HR Info Card Styles (Enhanced)
  hrInfoCard: {
    marginBottom: 16,
    backgroundColor: "#f0f4ff",
    elevation: 0,
    borderRadius: 12,
    borderWidth: 1,
    borderColor: "#d4d4ff",
  },
  hrInfoCardContent: {
    padding: 16,
  },
  hrInfoHeader: {
    flexDirection: "row",
    alignItems: "center",
    gap: 12,
    marginBottom: 16,
    paddingBottom: 12,
    borderBottomWidth: 1,
    borderBottomColor: "#d4d4ff",
  },
  hrInfoTitle: {
    fontWeight: "700",
    color: "#6200ee",
  },
  hrInfoDetails: {
    gap: 12,
  },
  hrInfoRow: {
    flexDirection: "row",
    alignItems: "center",
    gap: 12,
    padding: 10,
    backgroundColor: "#ffffff",
    borderRadius: 8,
  },
  hrInfoTextContainer: {
    flex: 1,
  },
  hrInfoLabel: {
    color: "#6b7280",
    marginBottom: 2,
    fontSize: 11,
  },
  hrInfoValue: {
    color: "#111827",
    fontWeight: "600",
    fontSize: 13,
  },
  detailsDivider: {
    marginVertical: 16,
  },
  detailsSection: {
    marginBottom: 24,
  },
  detailsSectionTitle: {
    fontWeight: "700",
    color: "#111827",
    marginBottom: 12,
  },
  detailsText: {
    color: "#6b7280",
    lineHeight: 22,
  },
  detailsInfoGrid: {
    gap: 12,
  },
  detailsInfoItem: {
    flexDirection: "row",
    alignItems: "center",
    gap: 12,
    padding: 12,
    backgroundColor: "#f8f9fa",
    borderRadius: 8,
  },
  detailsInfoLabel: {
    color: "#6b7280",
    marginBottom: 2,
  },
  detailsInfoValue: {
    color: "#111827",
    fontWeight: "600",
  },
  detailsSkillsContainer: {
    flexDirection: "row",
    flexWrap: "wrap",
    gap: 8,
  },
  detailsSkillChip: {
    backgroundColor: "#ede9fe",
  },
  detailsSkillChipText: {
    color: "#7c3aed",
    fontWeight: "600",
  },
  detailsActions: {
    marginTop: 8,
    marginBottom: 32,
  },
  viewApplicationsButton: {
    borderRadius: 12,
    marginBottom: 12,
  },
  readOnlyNote: {
    textAlign: "center",
    color: "#f59e0b",
    fontStyle: "italic",
  },
  // Skeleton Styles
  skeleton: {
    backgroundColor: "#e0e0e0",
    borderRadius: 4,
  },
  skeletonTitle: {
    width: "60%",
    height: 20,
  },
  skeletonChip: {
    width: 70,
    height: 30,
    borderRadius: 6,
  },
  skeletonHrInfo: {
    width: "100%",
    height: 50,
    marginBottom: 10,
    borderRadius: 8,
  },
  skeletonDescription: {
    width: "100%",
    height: 16,
    marginBottom: 6,
  },
  skeletonDescription2: {
    width: "80%",
    height: 16,
    marginBottom: 10,
  },
  skeletonMeta: {
    width: 80,
    height: 14,
  },
  skeletonSkill: {
    width: 60,
    height: 30,
    borderRadius: 6,
  },
});
